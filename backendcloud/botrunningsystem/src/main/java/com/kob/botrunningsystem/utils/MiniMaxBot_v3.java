 ...
 * <p>
 * v2.0 优化
 * DEPTH(层数) 减少位置, 由max进入min, 修改为 min进入max时 -- (未测试)
 * <p>
 * v3.0 叶子节点添加
 * Monte Carlo Rollout
 * 每个节点重复n次 rollout, 得到此节点获胜或者失败的概率
 */
public class MiniMaxBot_v3 implements java.util.function.Supplier<Integer> {
    ...
    // 分数评估 层数
    private static final int SCOREDEPTH = 10;
    ...
    // 检验当前回合，长度是否增加  true 增加, 增加时-是尾部加长
    ...
    public static Integer nextMove(String input) {
        ...
        if (checkScore(g, aCells, bCells) == 0) { // 0种 表示已经输, 特殊处理, 无需minmax, 随便返回一个方向即可
            return 0;
        }
        if (checkScore(g, aCells, bCells) == 1)  // 1种 只能这样走, 特殊处理, 无需minmax, 返回此时能走的方向
            for (int i = 0; i < 4; i++) {
    ...
    public static int max(int[][] g, int depth, int alpha, int beta, int moveScore) {
        ...
        // 叶子节点 调用 Monte Carlo Rollout
        if (depth == 0) return monteCarloRollout(g, new ArrayList<>(aCells), new ArrayList<>(bCells));
...
//        int score = checkScore(g, aCells, bCells); // 计算分数
    ...
    // Monte Carlo Rollout 叶子节点,随机数移动, 直到对局结束
    private static int monteCarloRollout(int[][] g, List<Cell> playerCells, List<Cell> foe) {

        // 成功 或者 失败
        if(moveNumber(g, foe) == 0) {
            return WINSCORE;
        }
        if(moveNumber(g, playerCells) == 0) {
            return LOSESCORE;
        }
        ...
        int playerX = playerCells.get(playerCells.size() - 1).x;
        int playerY = playerCells.get(playerCells.size() - 1).y;

        int foeX = playerCells.get(foe.size() - 1).x;
        int foeY = playerCells.get(foe.size() - 1).y;

        int nextPX, nextPY, nextFX, nextFY;

        while (true) {
            if (isMove(g, nextPX = playerX + dx[move = random.nextInt(4)], nextPY = playerY + dy[move])
                    && isMove(g, nextFX = foeX + dx[move = random.nextInt(4)], nextFY = foeY + dy[move])) { // 随机下一个可移动的位置
                g[nextPX][nextPY] = 1;
                g[nextFX][nextFY] = 1;
                playerCells.add(new Cell(nextPX, nextPY));
                foe.add(new Cell(nextFX, nextFY));
                break;
            }
        }
        monteCarloRollout(g, playerCells, foe);
