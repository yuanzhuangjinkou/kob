...
public class MiniMaxBot_v1 implements java.util.function.Supplier<Integer> {
    ...
    private static List<Cell> aCells = new LinkedList<Cell>();
    ...
    public static int max(int[][] g, int depth, int alpha, int beta, int moveScore) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (!checkTailIncreasing(step)) { // 长度不增加, 删除尾巴
                aCells.remove(0);
            ...
            int value = min(g, depth - 1, alpha, beta, moveScore, score);
            // 还原现场 (可以不用)
            g[x][y] = 0;
...
//        int score = checkScore(g, aCells, bCells); // 计算分数
        ...
        for (int i = 0; i < 4; i++) {
            ...
            int value = max(g, depth, alpha, beta, moveScore);
            // 还原现场
            g[x][y] = 0;
